...
/**
 * Parse KiCad schematic content to extract component and net metadata.
 * This is useful for extracting metadata from schematics generated by external
 * tools like the MAPO pipeline.
 */
export function parseKicadSchematic(content: string): {
  components: SchematicComponent[];
  nets: SchematicNet[];
  sheets: SchematicSheet[];
} {
  ...
  // Extract root UUID for sheet
  const rootUuidMatch = content.match(/\(kicad_sch[^)]*\(uuid\s+"([^"]+)"\)/);
  if (rootUuidMatch) {
    sheets.push({
      name: 'Root',
      uuid: rootUuidMatch[1],
      page: 1,
    });
  }
  ...
  // If we found wires but no labels, create implicit net entries based on wire count
  // This gives a better indication of connectivity
  if (wireMatches.length > 0 && nets.length === 0) {
    nets.push({
      name: 'Implicit Nets',
      uuid: randomUUID(),
      code: 1,
  ...
  log.info('Parsed KiCad schematic', {
    componentCount: components.length,
    netCount: nets.length,
    wireCount: wireMatches.length,
    sheetCount: sheets.length,
  });

  return { components, nets, sheets };
}
